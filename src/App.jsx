...
function App() {
    ...
    React.useEffect(() => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    data.forEach(order => {
                        ...
                        // Case A: Finished Report (Mobile has measured)
                        if (xml.includes('<MeasurementReport')) {
                            // Check if already processed
                            if (!processedReportIds.current.has(order.id)) {
                                console.log("New Report Received:", order.id);
                                handleMeasurementXml(xml);
                                processedReportIds.current.add(order.id);
                            }
                        }
                        // Case B: Pending Request (From SYNK or Test)
                        else if (xml.includes('<MeasurementRequest') || xml.includes('<Order>')) {
                            // Helper to parse definitions from XML string
                            const parser = new DOMParser();
                            const doc = parser.parseFromString(xml, "text/xml");

                            const definitions = Array.from(doc.querySelectorAll('Param')).map(p => ({
                                id: p.getAttribute('ID'),
                                nominal: p.getAttribute('Nominal'),
                                upperTol: p.getAttribute('TolUp'),
                                lowerTol: p.getAttribute('TolLo'),
                                gdtType: p.getAttribute('GD') || 'none',
                                isActive: true
                            }));

                            // Only add if it's not generated by US (we have local state), OR just sync everything?
                            // For simplicity, we sync everything but check duplicates in state setter.
                            inboxRequests.push({
                                id: order.id,
                                ...
                                status: order.status,
                                definitions: definitions, // Pass parsed definitions!
                                rawData: xml
                            });
                        }
                    });

                    // Update Inbox with Requests (deduplicating against existing IDs is good practice)
                    setMeasurementRequests(prev => {
                        // Merge prev and new, new overwrites prev (priority to server source)
                        // But wait, if we created it locally, we want to keep it.
                        // Actually, let's just use the server list as truth for "External" requests.
                        // But local requests might not be on server immediately? 
                        // "Perfect Sync" implies server is source of truth.

                        // Simple approach: Only add ones we don't have.
                        const newUnique = inboxRequests.filter(r => !prev.some(p => p.id === r.id));
        ...
        const interval = setInterval(fetchOrders, 2000); // Poll faster (2s)
