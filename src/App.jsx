...
function App() {
    ...
    const handleOrderCompletion = (order) => {
        ...
        // 3. Move from Active to Archive (Fixing missing data in ControlMeasurement)
        setMeasurementRequests(prev => prev.filter(r => r.id !== order.id));
        setArchivedRequests(prev => {
            if (prev.some(r => r.id === order.id)) return prev;
            const archivedReq = {
                ...order, // Contains all fields including drawingNumber etc
                archivedAt: order.completedAt,
                parameters: order.results, // Ensure results are burned in
                status: isAllOk ? 'OK' : 'FAIL',
                serialNumber: matkort.serialNumber
            };
            return [archivedReq, ...prev];
        });
    };
